...
// 3. API TRA CỨU: MỘ PHẦN (Đã hỗ trợ tìm không dấu / có dấu toàn diện)
app.get('/api/martyrs', async (req, res) => {
    try {
        ...
        if (name) { 
            sql += ` AND unaccent(ho_va_ten) ILIKE unaccent($${paramIndex})`; 
            values.push(`%${name}%`); 
            paramIndex++; 
        }
        if (birth) { 
        ...
        if (home) { 
            sql += ` AND unaccent(que_quan) ILIKE unaccent($${paramIndex})`; 
            values.push(`%${home}%`); 
            paramIndex++; 
        }
        if (area) { 
...
// 4. API TRA CỨU: TRONG ĐỀN THỜ (Đã hỗ trợ tìm không dấu / có dấu toàn diện)
